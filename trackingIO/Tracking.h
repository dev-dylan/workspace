...
@interface Tracking(GX_CAID)

//true：开启广协CAID false：关闭，默认开启，提高归因维度与准确度 如需自己接入广协CAID传入false关闭广协CAID。如需调用请在
//initWithAppKey:withChannelId: 函数调用前调用该函数。
+(void)enableGX_CAID:(BOOL)enable;
